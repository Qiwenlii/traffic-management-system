...
public class Intersection {
    ...
    /** traffic light with a yellow time and duration time. */
    private IntersectionLights allLights;
     ...
     * otherwise.
     *
     * @return boolean of whether this intersection has traffic lights
     */
    ...
    public void setLightDuration(int duration) {
        ...
        if (duration < allLights.getYellowTime() + 1) {
            throw new IllegalArgumentException("duration is less than the " +
                    "traffic lights' yellow time plus one");
        }
        allLights.setDuration(duration);
    }

    /**
     * Adds traffic lights to this intersection the given route order.
     * A new TrafficLight signal should be added to each incoming route and
     * a new IntersectionLights instance should be added to this intersection.
     *
     * The traffic lights will go green for incoming routes in the order
     * specified.
     *
     * If any of the below conditions are true, then the method should throw
     * an exception as detailed below and make no changes to this intersection:
     *
     * The traffic light yellow time is less than one (1) second
     * The traffic light duration is less than the given yellowTime plus
     * one (1) second
     ...
     * i.e. {A,B,D,C,E} is a permutation of {A,B,C,D,E} but {X,Y,Z,K}, {A,B,C}
     * and {A,B,C,D,E,E} are not.
     */
    public void addTrafficLights(List<Route> order, int yellowTime, int duration)
        ...
        if (yellowTime < 1 || duration < yellowTime + 1) {
            throw new InvalidOrderException("yellowTime < 1; or duration < " +
                    "yellowTime + 1");
        }
        if (order.size() == 0) {
        ...
        // create a List to record the "order" routes.
        List<String> copyOrder = new ArrayList<>();
        // create a List to record the intersection's routes.
        List<String> copyRoute = new ArrayList<>();
        for (Route demo : order) {
            copyOrder.add(demo.toString());
        }
        for (Route demo : incomingConnections) {
            copyRoute.add(demo.toString());
        }
        Collections.sort(copyOrder);
        Collections.sort(copyRoute);
        // if the given order is a permutation or incomingRoutes, then after
        // sorted they are same.
        if ( ! copyOrder.equals(copyRoute)) {
            throw new IllegalArgumentException("the given order is not a " +
                    "permutation or incomingRoutes");
        }

        for (Route demo : order) {
            demo.addTrafficLight();
        }
        allLights = new IntersectionLights(order,yellowTime,duration);
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (this == obj || this.id.equals(((Intersection) obj).id)) {
            return true;
        } else {
            return false;
        }
    ...
    public String toString() {
        if (! hasTrafficLights()){
            ...
            return this.id + ":" + allLights.toString();
        }
    }
}
