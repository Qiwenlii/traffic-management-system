...
public class Route {
    ...
    /** Calculate the averaging congestion. */
    ...
    public int getCongestion() {
        calculator = new AveragingCongestionCalculator(this.getSensors());
     ...
     *
     * have the same identifier string
     * have the same default speed
     * either both have a traffic light signal or both have no traffic light
     * signal
     * have the same traffic light signal status (GREEN, RED, etc.)
     * either both have an electronic speed sign or both have no electronic
     * speed sign
     * have the same electronic speed sign speed
     * have the same sensors (comparison should make use of each sensor's
    ...
    public boolean equals(Object obj) {
        ...
        if (obj instanceof Route && this.id == ((Route) obj).id
                && this.defaultSpeed == ((Route) obj).defaultSpeed
                && this.getTrafficLight() == ((Route) obj).getTrafficLight()
                && this.hasSpeedSign() == ((Route) obj).hasSpeedSign()
                && this.getSpeed() == ((Route) obj).getSpeed()
                && this.getSensors().containsAll(((Route) obj).getSensors())
                && this.getSensors().size() == ((Route) obj).getSensors().size()) {
            return true;
        } else{
            return false;
        }
